...
const Button = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: string, size?: string, isDisabled?: boolean }>(
...
export function TooltipRtl() {
  ...
  return (
    <div className="flex flex-col gap-8 w-full items-center">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="gap-2">
              {languageNames[lang]} <ChevronDown size={16} />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            {Object.entries(languageNames).map(([key, name]) => (
              <DropdownMenuItem key={key} onClick={() => setLang(key)}>
                {name}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
      ...
      <div className="grid gap-4 w-full place-items-center" dir={dir}>
...
export function SocialMediaTooltips() {
  const socials = [
    { name: 'Twitter', icon: (
  ...
  return (
    <div className="flex items-center justify-center bg-white p-8 rounded-lg border-2 border-neo-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] w-full">
      <div className="flex flex-wrap justify-center items-center gap-4">
        {socials.map((s) => (
          <div key={s.name} className="group relative flex flex-col items-center">
            <div className="opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 ease-[cubic-bezier(0.47,2,0.41,1.5)] absolute bottom-full mb-4 pointer-events-none z-10">
...
export function InteractiveDirectionalTooltips() {
  const directions = [
  ...
  return (
    <div className="flex items-center justify-center bg-white p-12 rounded-lg border-2 border-neo-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] w-full">
      <div className="flex flex-wrap justify-center items-center gap-4">
        {directions.map((item) => (
